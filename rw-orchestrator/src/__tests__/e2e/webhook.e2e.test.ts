...
import { createWebhookPayload, createWebhookSignature, createIssueWithLabels } from "../fixtures/github-payloads";
import { createHttpRequest, mockDurableClient, mockInvocationContext } from "../helpers/test-utils";

describe("GitHub Webhook E2E Tests", () => {
  let durableClient: any;
  let context: any;
  ...
  beforeEach(() => {
    durableClient = mockDurableClient();
  ...
  describe("Scenario 1: Valid issue.labeled with stage label", () => {
    it("should start new orchestration with correct instanceId", async () => {
      ...
      const issue = createIssueWithLabels(issueNumber, ["type/problem", "stage/0-intake"]);
      ...
      const signature = createWebhookSignature(payloadJson, WEBHOOK_SECRET);

      const request = {
        ...createHttpRequest("POST", payloadJson, {
      ...
      expect((payload as any).issue?.number).toBe(issueNumber);
      expect((payload as any).label?.name).toBe("stage/0-intake");
    });
  });
  ...
  describe("Scenario 3: Invalid signature", () => {
    it("should reject webhook with invalid signature", async () => {
      const payload = createWebhookPayload("issues", "labeled", undefined, {
        name: EXPECTED_REPO,
        owner: { login: EXPECTED_OWNER }
      });
      ...
      expect(wrongSignature).not.toBe(createWebhookSignature(payloadJson, WEBHOOK_SECRET));
    });
  });
  ...
  describe("Scenario 4: Unsupported label event", () => {
    it("should ignore labeled event for non-stage label", async () => {
      const payload = createWebhookPayload(
        "issues",
        "labeled",
        createIssueWithLabels(1, ["type/problem", "documentation"]),
        {
          name: EXPECTED_REPO,
          owner: { login: EXPECTED_OWNER }
        },
      ...
      expect(payload.action).toBe("labeled");
      expect((payload as any).label?.name).not.toBe("stage/0-intake");
    });
  });
  ...
  describe("Scenario 5: Valid workflow_run.completed", () => {
    it("should raise workflowCompleted event to running orchestration", async () => {
      const issueNumber = 42;
      const payload = createWebhookPayload("workflow_run", "completed", undefined, {
        name: EXPECTED_REPO,
        owner: { login: EXPECTED_OWNER }
      });
  ...
  describe("Scenario 6: Missing x-github-event header", () => {
    it("should reject webhook without event header", async () => {
      const payload = createWebhookPayload("issues", "labeled");
      const payloadJson = JSON.stringify(payload);
      const signature = createWebhookSignature(payloadJson, WEBHOOK_SECRET);
  ...
  describe("Scenario 7: GET request (health check)", () => {
    it("should return version info on GET request", async () => {
      const request = createHttpRequest("GET");
  ...
  describe("Scenario 9: Issue unlabeled event", () => {
    it("should process unlabeled event for status labels", async () => {
      const payload = createWebhookPayload(
        "issues",
        "unlabeled",
        createIssueWithLabels(1, ["type/problem", "stage/0-intake"]),
        {
          name: EXPECTED_REPO,
          owner: { login: EXPECTED_OWNER }
        },
  ...
  describe("Scenario 10: Wrong repository", () => {
    it("should ignore webhook from non-configured repository", async () => {
      ...
      expect((payload as any).repository?.owner?.login).not.toBe(EXPECTED_OWNER);
    });
  });
});
