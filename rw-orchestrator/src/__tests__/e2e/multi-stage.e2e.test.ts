...
describe("Multi-Stage Orchestration E2E", () => {
  ...
  describe("Multi-Stage Happy Path: evidence-enrich → normalize → dedupe → software-fit", () => {
    it("should progress through all four stages with successful transitions", async () => {
      ...
      let issue = createIssueWithLabels(issueNumber, ["type/problem", "stage/0-evidence"]);
      ...
      let evidenceStage = STAGES.find((s) => s.id === "evidence-enrich")!;
      let hasExpected = evidenceStage.expectedNextLabels.some((label) =>
      ...
      hasExpected = normalizeStage.expectedNextLabels.some((label) =>
        retrieved.labels.some((l) => l.name === label)
      );
      expect(hasExpected).toBe(true);
  ...
  describe("Country-gated Path: software-fit → country-validation → score", () => {
    it("should route country-dependent issues through the country validation gate before scoring", async () => {
      ...
      let issue = createIssueWithLabels(issueNumber, ["type/problem", "stage/2-deduped"]);
      mockGitHub.setIssue(issue);
      ...
      retrieved = await mockGitHub.getIssue("owner", "repo", issueNumber);
      let fitStage = STAGES.find((s) => s.id === "software-fit")!;
      let hasExpected = fitStage.expectedNextLabels.some((label) =>
        retrieved.labels.some((l) => l.name === label)
      );
      expect(hasExpected).toBe(true);
      ...
      hasExpected = countryValidationStage.expectedNextLabels.some((label) =>
        retrieved.labels.some((l) => l.name === label)
      );
      expect(hasExpected).toBe(true);
