...
describe("Single-Stage Orchestration E2E", () => {
    ...
    describe("Happy Path 0a: Evidence Enrich Stage → Advance to stage/0-intake", () => {
        it("should progress from stage/0-evidence to stage/0-intake", async () => {
            const initialIssue = (0, github_payloads_1.createIssueWithLabels)(41, ["type/problem", "stage/0-evidence"]);
            ...
            const updatedIssue = (0, github_payloads_1.createIssuePayload)(41, ["type/problem", "stage/0-intake", "rw/processing", "rw/stage-evidence-enrich"], null, "Test Issue");
    ...
    describe("Happy Path 3a: Country Validation → Terminal (status/needs-info)", () => {
        it("should treat a country-validation hold as a terminal successful outcome", async () => {
            const initialIssue = (0, github_payloads_1.createIssueWithLabels)(49, ["type/problem", "stage/2.5-country-validation"]);
            mockGitHub.setIssue(initialIssue);
            const retrieved = await mockGitHub.getIssue("owner", "repo", 49);
            const stage = (0, stageDecision_1.decideNextStage)(retrieved, stages_1.STAGES);
            expect(stage?.id).toBe("country-validation");
            const terminatedIssue = (0, github_payloads_1.createIssuePayload)(49, ["type/problem", "stage/2.5-country-validation", "status/needs-info", "rw/processing"], null);
            mockGitHub.setIssue(terminatedIssue);
            ...
            const hasExpected = countryValidationStage.expectedNextLabels.some((label) => verified.labels.some((l) => l.name === label));
            expect(hasExpected).toBe(true);
